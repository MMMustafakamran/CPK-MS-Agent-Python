...
| **10** | **Voiceover Audio Muxing & Codec** | Screen recordings are silent by default; adding voiceover tracks to specific demos. WebM container requires `libopus` or `vorbis` audio (not `aac`), and FFmpeg must be installed on the runner. | Install `ffmpeg` (`sudo apt-get install -y ffmpeg`), store audio in tracked `autorecorder/audio/`, and mux via `ffmpeg -i video.webm -i audio.m4a -c:v copy -c:a libopus -map 0:v:0 -map 1:a:0 -shortest` in consolidation step. |
